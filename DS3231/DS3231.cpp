/****************************************
16/07/2018
implementation of Alarm 1 functions by Thomas Lips 
****************************************/
...
//add regs for alarm
#define REG_A1SEC	0x07
#define REG_A1MIN	0x08
#define REG_A1HOUR	0x09
#define REG_A1DAY_DATE	0x0a
...
void DS3231::turnOnA1(){
	uint8_t con = _readRegister(REG_CON);
	con |= 0b00000101;
	Serial.print("CON: ");
	Serial.print(con);
	Serial.print("\n");
	_writeRegister(REG_CON, con);
	_writeRegister(REG_STATUS, 0b11111110 & _readRegister(REG_STATUS)); //explicit clear of A1F
}

void DS3231::turnOffA1(){
	uint8_t con = _readRegister(REG_CON);
	con &= 0b11111110;
	_writeRegister(REG_CON, con);
}

void DS3231::setModeA1(uint8_t mode){
	if (mode <= AL_HR_MIN_SEC_MATCH) {
		_writeRegister(REG_A1DAY_DATE, _readRegister(REG_A1DAY_DATE) | 0b10000000);
	}
	else {
		_writeRegister(REG_A1DAY_DATE, _readRegister(REG_A1DAY_DATE) & 0b01111111);
	}

	if (mode <= AL_MIN_SEC_MATCH) {
		_writeRegister(REG_A1HOUR , _readRegister(REG_A1HOUR) | 0b10000000);
	}
	else {
		_writeRegister(REG_A1HOUR, _readRegister(REG_A1HOUR) & 0b01111111);
	}

	if (mode <= AL_SECOND_MATCH) {
		_writeRegister(REG_A1MIN, _readRegister(REG_A1MIN) | 0b10000000);

	}
	else {
		_writeRegister(REG_A1MIN, _readRegister(REG_A1MIN) & 0b01111111);
	}

	if (mode <= AL_ONCE_SECOND) {
		_writeRegister(REG_A1SEC, _readRegister(REG_A1SEC) | 0b10000000);
	}
	else {
		_writeRegister(REG_A1SEC, _readRegister(REG_A1SEC) & 0b01111111);
	}

}

void DS3231::setTimeA1(uint8_t day_date, bool day, uint8_t hour, uint8_t min, uint8_t sec){
	if (hour < 25 && hour >= 0 && min < 61 && min >= 0 && sec < 61 && sec >= 0 ) {
		//set seconds
		uint8_t encoded = _encode(sec);
		uint8_t reg_new = (_readRegister(REG_A1SEC) / 128)*128 + encoded;
		_writeRegister(REG_A1SEC, reg_new);
		//set minutes
		encoded = _encode(min);
		reg_new = (_readRegister(REG_A1MIN) / 128) * 128 + encoded;
...
//sets AF1 bit to zero, needs to be called after each interrupt
void DS3231::resetA1(){
	_writeRegister(REG_STATUS,_readRegister(REG_STATUS)& ~_BV(0));
}

bool DS3231::read_A1F()
{
	return _readRegister(REG_STATUS) & 0b00000001;
}
